...
Emulator::Emulator(const std::filesystem::path& command_line,
  ...
  // guide the first time they ever open the emulator (MessageBox is unavailable
...
X_STATUS Emulator::Setup(
  ...
  // Initialize the GPU.
  XELOGI("{}: Before CreateGraphicsSystem()", __func__);
  ...
  XELOGI("{}: Before KernelState create", __func__);
  // Shared kernel state.
  kernel_state_ = std::make_unique<xe::kernel::KernelState>(this);
  XELOGI("{}: After KernelState create", __func__);
...
X_STATUS Emulator::LaunchXexFile(const std::filesystem::path& path) {
  ...
  // JTAG/hdd-style dumps ship their DLC/updates in a "content" folder next to
  // the executable (the real console's Hdd1:\Content). The game accesses those
  // files through the mounted Harddisk0 device, but license/enumeration
  // queries go through XAM's ContentManager, which looks at the host content
  // root - so redirect it to the game-local folder when one is present, and
  // both views line up.
  std::error_code ec;
  auto game_local_content = path.parent_path() / "content";
  if (!std::filesystem::exists(game_local_content, ec)) {
    game_local_content = path.parent_path() / "Content";
  }
  if (std::filesystem::exists(game_local_content, ec) &&
      std::filesystem::is_directory(game_local_content, ec)) {
    kernel_state_->xam_state()->content_manager()->SetRootPath(
        game_local_content);
  }
...
bool Emulator::ExceptionCallback(Exception* ex) {
...
#if XE_PLATFORM_WINRT
  // Self-heal for execute-DEP inside the code cache: if a page lost its
  // execute right (e.g. a protection race while the cache was growing),
  // restore RWX and retry instead of crashing. Bail out if the same PC keeps
  // faulting - then it's a real crash, fall through to the dump below.
  if (ex->code() == Exception::Code::kAccessViolation &&
      ex->fault_address() == ex->pc()) {
    static std::atomic<uint64_t> last_heal_pc{0};
    static std::atomic<uint32_t> same_pc_heal_count{0};
    uint32_t repeats = (last_heal_pc.exchange(ex->pc()) == ex->pc())
                           ? same_pc_heal_count.fetch_add(1) + 1
                           : (same_pc_heal_count.store(0), 0u);
    if (repeats < 3) {
      void* page = reinterpret_cast<void*>(ex->pc() & ~uintptr_t(0xFFF));
      if (xe::memory::Protect(page, 0x1000,
                              xe::memory::PageAccess::kExecuteReadWrite,
                              nullptr)) {
        XELOGW(
            "ExceptionCallback: restored EXECUTE on code cache page {} (PC "
            "0x{:X}, attempt {}) - protection was lost at runtime",
            page, uint64_t(ex->pc()), repeats + 1);
  ...
  // May legitimately be null: the PC can be inside the backend's host
  // thunks/helpers, which live in the code cache range but have no guest
  // function. Dereferencing unconditionally would crash inside the crash
  // handler and mask the original exception.
  ...
  if (guest_function) {
    ...
    crash_msg.append(fmt::format(
        "PC: host 0x{:016X} (backend thunk/helper, no guest function)\n",
        uint64_t(ex->pc())));
  }
  if (ex->code() == Exception::Code::kAccessViolation) {
