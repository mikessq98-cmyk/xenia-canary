...
class CodeCacheBase : public CodeCache {
 public:
  ~CodeCacheBase() override {
    ...
    if (mapping_ != xe::memory::kFileMappingHandleInvalid) {
      ...
      // The UWP code cache is a private RWX allocation, not a file mapping.
 ...
 protected:
  ...
  bool Initialize() {
...
#if XE_PLATFORM_WINRT
    // On Xbox the OS enforces W^X for section mappings: MapViewOfFile3FromApp
    // rejects PAGE_EXECUTE_READWRITE views, and the separate write+execute
    // views of upstream's fallback cannot be used because the emitter
    // finalizes relocations relative to the write view - cross-function near
    // rel32 branches (e.g. call guest_to_host_thunk) then land at
    // target - (write_base - execute_base) when run from the execute view
    // (observed as an execute-DEP crash at 0x90000160). Private memory CAN be
    // made RWX with the codeGeneration capability (VirtualAllocFromApp +
    // VirtualProtectFromApp), which is how UWP emulators JIT on Xbox, so
    // allocate the code cache as a plain private reservation instead of a
    // file mapping; EnsureCommitted commits and protects it RWX chunk by
    // chunk through xe::memory::AllocFixed.
    ...
    if (!generated_code_execute_base_) {
      XELOGE(
          "Unable to reserve the private RWX code cache at {:X}",
...
#if XE_PLATFORM_WINRT
    // The exact bases matter for the JIT indirection slots (32-bit host
    // addresses); log them to catch a mapping landing away from the requested
    // fixed address or losing execute rights in the App Container.
    XELOGI(
        "CodeCache: execute_base={} write_base={} (rwx_preferred={}) "
        "indirection_table_base={}",
        static_cast<void*>(generated_code_execute_base_),
        static_cast<void*>(generated_code_write_base_),
        xe::memory::IsWritableExecutableMemoryPreferred() ? 1 : 0,
  ...
  void EnsureCommitted(size_t high_mark) {
    ...
    do {
...
#if XE_PLATFORM_WINRT
      // Commit only the NEW chunk, not the whole [0, new_mark) range: on UWP
      // committing executable memory is a two-step VirtualAllocFromApp(RW) +
      // VirtualProtectFromApp(RWX), so re-running it over already-live pages
      // would briefly drop their execute right while other threads are running
      ...
      if (generated_code_execute_base_ == generated_code_write_base_) {
        ...
        xe::memory::AllocFixed(generated_code_write_base_ + old_commit_mark,
                               new_commit_mark - old_commit_mark,
                               xe::memory::AllocationType::kCommit,
                               xe::memory::PageAccess::kReadWrite);
      }
