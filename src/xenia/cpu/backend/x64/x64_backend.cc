...
bool X64Backend::Initialize(Processor* processor) {
  ...
  XELOGI("X64Backend thunks: host_to_guest={} guest_to_host={} resolve={}",
         static_cast<void*>(host_to_guest_thunk_),
         static_cast<void*>(guest_to_host_thunk_),
         static_cast<void*>(resolve_function_thunk_));
  if (uint64_t(resolve_function_thunk_) >> 32) {
    XELOGE(
        "X64Backend: resolve thunk above 4GB - indirection slots would "
        "truncate it; the code cache mapping is misplaced");
  }
#endif
