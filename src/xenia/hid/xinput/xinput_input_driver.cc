...
X_STATUS XInputInputDriver::Setup() {
  ...
  // LoadLibrary of xinput1_4.dll is forbidden in the App Container. Link
  // xinputuap.lib and bind the exports directly. XInputGetStateEx (guide
  // button, ordinal 100) and XInputEnable are not part of the UAP variant, so
  // the guide button falls back to the regular state (its bit is simply absent).
  module_ = nullptr;  // nothing to FreeLibrary on shutdown
  XInputGetCapabilities_ = reinterpret_cast<void*>(&XInputGetCapabilities);
  XInputGetState_ = reinterpret_cast<void*>(&XInputGetState);
  XInputGetStateEx_ = reinterpret_cast<void*>(&XInputGetState);
  XInputGetKeystroke_ = reinterpret_cast<void*>(&XInputGetKeystroke);
  XInputSetState_ = reinterpret_cast<void*>(&XInputSetState);
  XInputEnable_ = nullptr;
