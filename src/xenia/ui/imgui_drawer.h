...
class ImGuiDrawer : public WindowInputListener, public UIDrawer {
 public:
  ...
  // Whether the ImGui main menu is currently shown on UWP. While a game runs
  // the menu is hidden and the drawer is unregistered from the presenter (the
  // guest output thread then presents directly, exactly like the desktop build
  // with no dialogs = full speed); the input listener stays registered so the
...
#if XE_PLATFORM_WINRT
  bool uwp_menu_visible_ = true;
  // Whether this drawer is currently registered as a UI drawer with the
  // presenter (menu visible, or dialogs/notifications present).
  bool uwp_ui_drawer_registered_ = false;
#endif
