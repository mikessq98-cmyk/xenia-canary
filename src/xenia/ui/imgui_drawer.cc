...
#if XE_PLATFORM_WINRT
  // On UWP the ImGui main menu is drawn by this drawer (no native menu bar).
  // The input listener is registered for the whole presenter lifetime (so the
  // gamepad View button can toggle the menu even while it's hidden); the UI
  // drawer registration follows whether there is anything to draw - the menu,
  // dialogs or notifications. With none of those the drawer is unregistered
  // and the guest output thread presents the game directly (desktop behavior).
  if (presenter_) {
    if (presenter_ == new_presenter) {
      return;
    }
    if (uwp_ui_drawer_registered_) {
...
void ImGuiDrawer::SetUWPMenuVisible(bool visible) {
  ...
  // Register while there is anything to draw: the menu, a dialog or a
  // notification. Otherwise unregister so the guest output thread presents
...
void ImGuiDrawer::Draw(UIDrawContext& ui_draw_context) {
  ...
  if (!dialogs_.empty()
#if XE_PLATFORM_WINRT
      // On UWP the always-on menu needs gamepad navigation even with no dialog.
      || force_uwp_render
#endif
  ) {
    UpdateGamepads();
...
#if XE_PLATFORM_WINRT
  // CoreWindow has no native menu bar; draw Xenia's main menu with ImGui so it
  // is visible and gamepad-navigable on Xbox. Hidden while a game runs (toggle
  // with the View button).
  if (uwp_menu_visible_) {
...
#if XE_PLATFORM_WINRT
  // Report whether ImGui wants text input this frame so the UWP window can
  // show/hide the system on-screen keyboard (gamepad-navigable on Xbox).
  if (window_) {
    window_->SetImGuiWantsTextInput(io.WantTextInput);
  }
#endif
...
#if XE_PLATFORM_WINRT
  // On Xbox the gamepad View button (delivered by the CoreWindow as a virtual
  // key) toggles the ImGui main menu - but never while a game is running (the
  // menu is fully suppressed then so it can't sit on top of the game and eat
  // frame time; the View button is also used for the hold-to-open-keyboard
  // gesture handled by the window itself).
  if (e.virtual_key() == VirtualKey::kXboxOneGamepadView) {
    if (window_ && !window_->uwp_menu_suppressed()) {
      SetUWPMenuVisible(!uwp_menu_visible_);
...
#if XE_PLATFORM_WINRT
  // Keep drawing while the UWP main menu is shown. When the menu is hidden
  // (a game is running) and the last dialog/notification went away, unregister
  // the UI drawer so the guest output thread presents directly - but keep the
  // input listener so the gamepad View button can reopen the menu.
  if (uwp_menu_visible_) {
    return;
  }
  if (presenter_ && uwp_ui_drawer_registered_) {
    presenter_->RemoveUIDrawerFromUIThread(this);
    uwp_ui_drawer_registered_ = false;
  }
  return;
