...
class Window {
 public:
  ...
  // Public accessor for the main menu (used by the UWP ImGui menu renderer,
  // which has no native menu bar). GetMainMenu() exists but is protected.
  MenuItem* main_menu() const { return main_menu_.get(); }

#if XE_PLATFORM_WINRT
  // Updated by the ImGui drawer every frame; the UWP window implementation
  // reflects it on the system on-screen keyboard (CoreInputView). Kept here
  // rather than read from ImGui::GetIO() so it doesn't depend on which ImGui
  // context happens to be current outside the drawer.
  ...
  void SetUWPMenuSuppressed(bool suppressed) {
    uwp_menu_suppressed_ = suppressed;
  }
