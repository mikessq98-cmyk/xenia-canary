...
bool D3D12Provider::EnableIncreaseBasePriorityPrivilege() {
  ...
  // OpenProcessToken/AdjustTokenPrivileges are not usable in the App Container;
  // the global-realtime queue priority isn't grantable there. Caller falls back
  // to HIGH priority on false.
  return false;
#else
...
#if XE_PLATFORM_WINRT
  // App Container can't LoadLibrary dxgi.dll / D3D12.dll. Both are guaranteed
  // present on Xbox / Win10+, so link dxgi.lib + d3d12.lib and bind the core
  // entry points directly. The library handles stay null (shutdown skips them).
  library_dxgi_ = nullptr;
  library_d3d12_ = nullptr;
  pfn_create_dxgi_factory2_ = &CreateDXGIFactory2;
  pfn_dxgi_get_debug_interface1_ = &DXGIGetDebugInterface1;
  pfn_d3d12_get_debug_interface_ = &D3D12GetDebugInterface;
  pfn_d3d12_create_device_ = &D3D12CreateDevice;
  pfn_d3d12_serialize_root_signature_ = &D3D12SerializeRootSignature;
#else
...
#if XE_PLATFORM_WINRT
  // Enable Device Removed Extended Data (DRED) so a GPU driver crash
  // (DXGI_ERROR_DRIVER_INTERNAL_ERROR 0x887A0020 observed on the console) can
  // be attributed to the last GPU operation via auto-breadcrumbs; the data is
  // read and logged where the removal is detected (pipeline_cache).
  {
    ...
    if (SUCCEEDED(
      ...
      XELOGI("D3D12: DRED auto-breadcrumbs and page fault tracking enabled");
    } else {
      XELOGW("D3D12: DRED settings interface unavailable");
    }
  }
#endif  // XE_PLATFORM_WINRT
