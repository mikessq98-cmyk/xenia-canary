...
#if XE_PLATFORM_WINRT
    // On Xbox there is no reliable way to interact with the first-run dialog
    // (it can cover the whole screen before input works), and text entry needs
    // the on-screen keyboard. Create and log in a default profile
    // automatically instead; more can be added later from the Profile menu.
    if (profile_manager->CreateProfile("default", /*autologin=*/true,
                                       /*default_xuid=*/true)) {
      XELOGI("Created and signed in the default profile automatically");
    } else {
      XELOGE("Failed to auto-create the default profile");
    }
#else
...
bool EmulatorWindow::Initialize() {
  ...
  auto console_menu = MenuItem::Create(MenuItem::Type::kPopup, "&Console");
  {
    ...
    console_menu->AddChild(MenuItem::Create(
        MenuItem::Type::kString, "&Settings editor (config.toml)", "",
        std::bind(&EmulatorWindow::ToggleSettingsEditorDialog, this)));
  }
...
void EmulatorWindow::FileOpen() {
...
#if XE_PLATFORM_WINRT
  // The WinRT picker is async; deliver the pick on the UI thread and run then.
  ui::ShowFileOpenPickerAsyncUWP(
      window_.get(), extensions, /*multi_selection=*/false,
  ...
  file_picker->set_extensions(extensions);
...
#if XE_PLATFORM_WINRT
  // Back to the frontend: lift the suppression and show the ImGui menu again.
  window_->SetUWPMenuSuppressed(false);
  imgui_drawer_->SetUWPMenuVisible(true);
#endif
}
...
#if XE_PLATFORM_WINRT
  ui::ShowFileOpenPickerAsyncUWP(
      window_.get(), extensions, /*multi_selection=*/true,
      [this](std::vector<std::filesystem::path> paths) {
        InstallContentPackages(std::move(paths));
      });
#else
  std::vector<std::filesystem::path> paths;
...
void EmulatorWindow::ToggleSettingsEditorDialog() {
...
#if XE_PLATFORM_WINRT
    // The game is now the foreground content: hide the ImGui menu and fully
    // suppress it so it cannot be opened (rendering it every frame on top of
    // the game costs frame time); it comes back when the title is closed.
    window_->SetUWPMenuSuppressed(true);
    imgui_drawer_->SetUWPMenuVisible(false);
#endif
  }
