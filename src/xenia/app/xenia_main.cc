...
// SDL is not built for the UWP App Container (XAudio2 covers audio there).
...
// Vulkan is unavailable in the UWP App Container (no loader / vulkan-1.dll load
// is blocked), so the UWP build ships D3D12 only.
#if !XE_PLATFORM_MAC && !XE_PLATFORM_WINRT
...
#endif  // !XE_PLATFORM_ANDROID && !XE_PLATFORM_WINRT
#if XE_PLATFORM_WIN32
// winkey uses GetAsyncKeyState/raw input, unavailable in the App Container;
// CoreWindow delivers keyboard input on UWP instead.
    ...
    std::vector<std::unique_ptr<T>> CreateAll(const std::string_view name,
      ...
      if (name.empty() || name == "any") {
        for (const auto& creator : creators_) {
          ...
          // Skip xinput for "any" and use SDL - except on UWP, where SDL is not
          // built and XInput(UAP) is the only gamepad driver.
#if !XE_PLATFORM_WINRT
