...
ContentManager::ContentManager(KernelState* kernel_state,
  ...
  XELOGI("XamState: ContentManager root_path={}", xe::path_to_utf8(root_path_));
}

ContentManager::~ContentManager() = default;

void ContentManager::SetRootPath(const std::filesystem::path& root_path) {
  root_path_ = root_path;
  XELOGI("ContentManager: root path redirected to {}",
         xe::path_to_utf8(root_path_));
}
...
X_RESULT ContentManager::WriteContentHeaderFile(const uint64_t xuid,
  ...
  // Non-throwing overloads: a guest save/content request must not be able to
  // take down the emulator with an unhandled std::filesystem_error (e.g. the
  // content root being on a path the App Container can't write to).
  std::error_code ec;
  if (!std::filesystem::exists(parent_path, ec)) {
    if (!std::filesystem::create_directories(parent_path, ec) || ec) {
      XELOGE("ContentManager: failed to create {} ({})",
             xe::path_to_utf8(parent_path), ec.message());
...
X_RESULT ContentManager::CreateContent(const std::string_view root_name,
  ...
  if (!std::filesystem::create_directories(package_path, ec) || ec) {
    XELOGE("ContentManager: failed to create {} ({})",
           xe::path_to_utf8(package_path), ec.message());
...
X_RESULT ContentManager::OpenContent(const std::string_view root_name,
  ...
  if (!std::filesystem::exists(package_path, open_ec)) {
    // Does not exist, must be created.
    XELOGW(
        "ContentManager::OpenContent('{}', xuid={:016X}, type={:08X}): "
        "package path {} does not exist",
        root_name, xuid, static_cast<uint32_t>(data.content_type.get()),
  ...
  XELOGI(
      "ContentManager::OpenContent('{}', xuid={:016X}, type={:08X}): mounted "
      "{} (license=0x{:08X})",
      root_name, xuid, static_cast<uint32_t>(data.content_type.get()),
      xe::path_to_utf8(package_path), content_license);
...
X_RESULT ContentManager::DeleteContent(const uint64_t xuid,
  ...
  auto package_path = ResolvePackagePath(xuid, data);
  std::error_code ec;
  if (std::filesystem::remove_all(package_path, ec) > 0 && !ec) {
    return X_ERROR_SUCCESS;
  } else {
    return X_ERROR_FILE_NOT_FOUND;
  }
