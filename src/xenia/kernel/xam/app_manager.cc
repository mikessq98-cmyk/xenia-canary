...
void AppManager::RegisterApps(KernelState* kernel_state, AppManager* manager) {
  XELOGI("XamState: AppManager::RegisterApps start");
  ...
  XELOGI("XamState: AppManager::RegisterApps done");
}

void AppManager::RegisterApp(std::unique_ptr<App> app) {
  XELOGI("XamState: AppManager::RegisterApp app_id=0x%08X", app->app_id());
