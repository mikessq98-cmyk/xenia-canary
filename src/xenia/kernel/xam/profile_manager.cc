...
ProfileManager::ProfileManager(KernelState* kernel_state,
  ...
  XELOGI("XamState: ProfileManager ctor start");
  logged_profiles_.clear();
  accounts_.clear();

  auto profiles = FindProfiles();
  XELOGI("XamState: ProfileManager found %u profiles", uint32_t(profiles.size()));
  for (const auto account_xuid : profiles) {
...
bool ProfileManager::LoadAccount(const uint64_t xuid) {
  ...
  XELOGI("XamState: ProfileManager::LoadAccount xuid={}", xuid_as_string);
...
bool ProfileManager::MountProfile(const uint64_t xuid, std::string mount_path) {
  ...
  XELOGI("XamState: ProfileManager::MountProfile xuid={:016X} path=%s",
         xuid, profile_path.string());
  auto device =
...
void ProfileManager::Login(const uint64_t xuid, const uint8_t user_index,
                           bool notify) {
  XELOGI("XamState: ProfileManager::Login xuid={:016X} user_index=%u notify=%u",
         xuid, user_index, notify);
