...
void KeyboardInputDialog::OnDraw(ImGuiIO& io) {
  ...
  if (ImGui::BeginPopupModal(title_.c_str(), nullptr,
    ...
    // The gamepad is the only text input device here, so draw our own keyboard
    // right in the dialog. The SYSTEM keyboard is deliberately not used: it is
    // a fullscreen overlay that stops our presents from completing (wedging
    // the UI thread, so typed characters arrived a whole session late), and
    // none of the events that report when it opens or closes are raised on
    // this console runtime.
    if (first_draw) {
      virtual_keyboard_.Reset();
    }
    ImGui::Spacing();
    std::string keyboard_text(text_buffer_.data());
    switch (virtual_keyboard_.Draw(keyboard_text, max_length_)) {
      case xe::ui::ImGuiVirtualKeyboard::Result::kAccept:
        input_submitted = true;
        break;
      case xe::ui::ImGuiVirtualKeyboard::Result::kCancel:
        text_ = "";
        cancelled_ = true;
        ImGui::CloseCurrentPopup();
        Close();
    ...
    xe::string_util::copy_truncating(text_buffer_.data(), keyboard_text,
                                     text_buffer_.size());
