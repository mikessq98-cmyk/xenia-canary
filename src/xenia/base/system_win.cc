...
void LaunchWebBrowser(const std::string_view url) {
  ...
  // ShellExecute is unavailable in the App Container.
  (void)url;
#else
...
#if XE_PLATFORM_WINRT
  // MessageBox is unavailable in the App Container; surface via the log. This
  // is also the FatalError path - flush so the message survives process exit.
  if (type == SimpleMessageBoxType::Error) {
    XELOGE("MessageBox (error): {}", message);
  } else {
    XELOGI("MessageBox: {}", message);
  }
  xe::FlushLog();
#else
  const wchar_t* title;
