...
#include <shlobj.h>  // SHGetKnownFolderPath; pulls desktop-only shobjidl_core.h
#endif
...
#if XE_PLATFORM_WINRT
// App Container forbids CreateFileW / FindFirstFileW / GetFileAttributesEx /
// SetFileAttributes. The *FromApp equivalents (here) operate on any path the app
// has been granted (LocalState/LocalCache, or a folder added to the
// FutureAccessList via the picker). Same semantics, App-Container-safe.
...
#if XE_PLATFORM_WINRT
  // SHGetKnownFolderPath is unavailable in the App Container; use the app's
  // private LocalState folder as the user/content root.
  try {
    auto local =
        winrt::Windows::Storage::ApplicationData::Current().LocalFolder();
    return std::filesystem::path(std::wstring(local.Path().c_str()));
  } catch (...) {
    return {};
  }
#else
...
bool CreateEmptyFile(const std::filesystem::path& path) {
  ...
  auto handle =
      CreateFile2FromAppW(path.c_str(), 0, 0, CREATE_ALWAYS, &params);
#else
...
std::unique_ptr<FileHandle> FileHandle::OpenExisting(
  ...
  HANDLE handle = CreateFile2FromAppW(path.c_str(), open_access, share_mode,
...
std::vector<FileInfo> ListFiles(const std::filesystem::path& path) {
  ...
  WIN32_FIND_DATA ffd;
#if XE_PLATFORM_WINRT
  HANDLE handle = FindFirstFileExFromAppW(
      (path / "*").c_str(), FindExInfoStandard, &ffd, FindExSearchNameMatch,
      nullptr, 0);
#else
...
#if XE_PLATFORM_WINRT
  return SetFileAttributesFromAppW(path.c_str(),
