...
// Xenia declares its own minimal XAudio2 ABI (api::), not <xaudio2.h>. On UWP we
// link xaudio2.lib (XAudio2.9, in-box on Win10+/Xbox); its real export is
// XAudio2CreateWithVersionInfo (the SDK's XAudio2Create is just an inline that
// forwards to it). Declare it with Xenia's ABI-compatible api:: types and thunk
// to the driver's stored function-pointer signature.
extern "C" HRESULT __stdcall XAudio2CreateWithVersionInfo(
    api::IXAudio2_8** out, UINT32 flags, api::XAUDIO2_PROCESSOR processor,
    DWORD ntddi_version);
static HRESULT __stdcall XAudio2CreateUwpThunk(
    api::IXAudio2_8** out, UINT32 flags, api::XAUDIO2_PROCESSOR processor) {
  return XAudio2CreateWithVersionInfo(out, flags, processor, NTDDI_VERSION);
}
#endif
...
#if XE_PLATFORM_WINRT
  // LoadLibrary of XAudio2_8.dll is forbidden in the App Container. Link
  // xaudio2.lib (XAudio2.9, in-box on Win10+/Xbox) and bind XAudio2Create
  // directly. xaudio2_module_ stays null so shutdown skips FreeLibrary.
  xaudio2_create_ = &XAudio2CreateUwpThunk;
  api_minor_version_ = 9;
#else
