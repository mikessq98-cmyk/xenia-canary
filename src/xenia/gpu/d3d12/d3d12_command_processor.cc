    ...
    if (!texture_cache_->IsDrawResolutionScaled() && written_length) {
      ...
      if (size > rb.sizes[write_index]) {
        ...
        if (SUCCEEDED(create_hr)) {
          ...
          static std::atomic<uint32_t> readback_fail_count{0};
          uint32_t n = readback_fail_count.fetch_add(1);
          if (n < 3 || (n % 100) == 0) {
            XELOGE("Failed to create a {} MB readback buffer (hr=0x{:08X}, {})",
                   size >> 20, uint32_t(create_hr), n + 1);
          }
          return true;
        }
      }
