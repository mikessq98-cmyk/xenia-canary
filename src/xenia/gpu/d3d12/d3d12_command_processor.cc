...
DEFINE_bool(
    d3d12_interpreter_render, false,
    "Draw one full-screen pass per frame with the microcode interpreter, over "
    "a real guest pixel shader, and pay for it.\n"
    "Nothing the game draws goes through it and it writes no colour - it shades "
    "pixels and discards them. It exists for the last unmeasured term in "
...
void D3D12CommandProcessor::LogHostMemoryStatistics() {
  ...
  if (pipeline_cache_) {
    ...
    // made it impossible to tell "nothing needed checking" from "the proven
    // list is not being kept across launches".
    XELOGI(
        "[MEM] shader pairs proven safe to execute: {} loaded at startup, {} "
        "proven this run",
        pipeline_cache_->solver_verified_at_startup(),
...
bool D3D12CommandProcessor::SetupContext() {
  ...
  RegisterMemoryArbiterConsumers();
...
#if XE_PLATFORM_WINRT
  // A pair quarantined earlier in THIS run must stop being drawn now, not from
  // the next launch. Skipping it at pipeline creation only covers pipelines not
  // built yet; the one that hung the GPU was built long before it did, so
  // without this the run keeps submitting the very draw it just proved fatal.
  if (pipeline_cache_->SolverIsShaderToxic(
          vertex_shader->ucode_data_hash(),
  ...
  *scisp = _mm_mullo_epi32(
  ...
  if (cvars::d3d12_serialize_draws_for_hang_diagnosis || verifying_new_pair ||
    ...
    if (!AwaitAllQueueOperationsCompletion()) {
      ...
      // still alive.
      pipeline_cache_->SolverQuarantineIsolatedExecutionHang(draw_vs_hash,
                                                             draw_ps_hash);
      xe::FlushLog();
      return false;
    }
...
void D3D12CommandProcessor::RegisterMemoryArbiterConsumers() {
  using ConsumerKind = GpuMemoryArbiter::ConsumerKind;

  // Cheapest to rebuild: translated bytecode is regenerated from the guest
  // microcode in milliseconds.
  memory_arbiter_.RegisterConsumer(
      ...
      /*free_unneeded=*/nullptr,
      // Host memory holding DXBC on the CPU. The GPU never sees it, and
      // translations referenced by a creation in flight are held back by their
      // own refcount, so this is safe to release at any point.
      /*trim_safe_mid_submission=*/true);

  // Pure scratch - pages come straight back when needed.
  memory_arbiter_.RegisterConsumer(
      ConsumerKind::kUploadPools,
      [this]() -> uint64_t {
        return constant_buffer_pool_
                   ? uint64_t(constant_buffer_pool_->GetAllocatedBytes())
                   : 0;
      },
      [this](uint64_t bytes_to_free) -> uint64_t {
      ...
      [this]() -> uint64_t {
        // Pages beyond what a frame actually uses are dead weight, not a
        // cache - nothing is faster for having kept them. Released on every
        // poll rather than waiting for a shortage.
        if (!constant_buffer_pool_) {
          return 0;
        }
        return uint64_t(
      ...
      // Only pages the GPU has already been through are released, and never
      // the one being filled - safe at any point.
      /*trim_safe_mid_submission=*/true);

  // Regenerated by the next resolve into the range.
  memory_arbiter_.RegisterConsumer(
      ConsumerKind::kIdleScaledResolve,
      [this]() { return texture_cache_->GetScaledResolveCommittedBytes(); },
      [this](uint64_t bytes_to_free) -> uint64_t {
        return texture_cache_->ReleaseIdleScaledResolveRegionsForArbiter(
            bytes_to_free);
      },
      [this]() -> uint64_t {
        // A region the game has stopped resolving into holds nothing anyone
        // can read - the next resolve regenerates it. Asking for 0 releases
        // exactly the regions that qualify as idle and nothing else.
        return texture_cache_->ReleaseIdleScaledResolveRegionsForArbiter(0);
      });

  // Recreated when the surface configuration comes back.
  memory_arbiter_.RegisterConsumer(
  ...
  // Released host resources kept only so a future texture can skip the
  // driver's creation cost. Nothing is using them by definition, so they are
  // given up long before any live texture - registered separately rather than
  // bundled into the texture consumer, so the memory report shows which of the
  // two is actually holding the memory.
  memory_arbiter_.RegisterConsumer(
      ConsumerKind::kResourceReusePool,
      [this]() { return texture_cache_->GetTextureResourcePoolBytes(); },
      [this](uint64_t bytes_to_free) -> uint64_t {
...
bool D3D12CommandProcessor::BeginSubmission(bool is_guest_command) {
  ...
  if (!submission_open_) {
    ...
    // One decision point for all GPU caches: polls the host occasionally and,
    ...
    // Hand the arbiter's verdict to the caches. They hold no thresholds of
    // their own - one poll of the host decides for all of them, which is what
    // stops them contradicting each other (a cache hitting its own ceiling
    // while gigabytes are free, or two of them trimming for the same shortage).
    if (texture_cache_) {
      texture_cache_->SetHostMemoryPressure(memory_arbiter_.pressure());
    }
