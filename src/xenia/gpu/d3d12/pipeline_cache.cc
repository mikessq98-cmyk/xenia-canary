...
PipelineCache::GetCurrentPixelShaderModification(
  ...
  if (render_target_cache_.GetPath() ==
    ...
    if (render_target_cache_.depth_float24_convert_in_pixel_shader() &&
      ...
      // Upstream's polygon offset mode takes precedence: it is about rendering
      // correctly (Z-fighting decals), while the hint below is only an
      // optimization, and d3d12_no_early_depth_stencil_hint only decides
      // whether to give that optimization up to avoid duplicate translations.
      if (apply_polygon_offset_in_shader) {
        modification.pixel.depth_stencil_mode =
            DepthStencilMode::kPolygonOffset;
      } else if (!cvars::d3d12_no_early_depth_stencil_hint &&
